...
- 프록시 모드는 해당 프록시를 사용하도록 설정한 앱(브라우저 등)에만 적용됩니다. GUI의 **시스템 프록시 자동 설정**(기본 켜짐)은 OS 전역 웹 프록시를 psdns로 지정해 OS 프록시를 따르는 앱 전반에 적용되지만, Firefox 등 자체 프록시 설정을 쓰는 앱은 제외됩니다. **macOS에서는 시스템 프록시 변경에 관리자 권한이 필요할 수 있어** 자동 설정이 거부되면 주소를 복사해 직접 넣어야 할 수 있습니다(Windows·Linux는 사용자 권한으로 동작). 비정상 종료로 설정이 남아도 다음 실행 때 자동으로 정리됩니다(그 사이에 프록시 설정을 직접 바꿨다면 psdns는 그 변경을 존중해 되돌리지 않습니다). **이미 다른 로컬 프록시(예: AdGuard 등 필터링 앱)가 시스템 프록시로 설정돼 실제로 동작 중이면** psdns는 이를 덮어쓰지 않고 자동 설정을 건너뛰며 안내 메시지를 표시합니다 — 그 앱을 끄거나, 자동 설정을 끄고 psdns 주소를 직접 넣으세요. 설정만 남고 아무것도 응답하지 않는 죽은 프록시(예: 비정상 종료한 이전 psdns의 잔재)는 자동으로 이어받습니다.
- **DNS 기반 광고 차단(AdGuard 등)과 함께 쓸 때:** psdns 프록시를 거친 요청은 이름 해석을 통신사 DNS가 아닌 psdns 내부 DoH로 직접 처리하므로, DNS 단계에서 광고를 막는 도구의 차단이 우회되어 광고가 다시 보일 수 있습니다(psdns의 핵심 동작인 DNS 우회의 부작용). 광고 차단을 유지하려면 `-doh`를 광고 차단 DoH 엔드포인트로 지정하세요(예: `psdns proxy -doh https://dns.adguard-dns.com/dns-query`, GUI는 고급 설정의 DoH 입력란). 그러면 psdns의 해석 자체가 광고 차단 DNS를 거쳐 **통신사 DNS 위조는 우회하면서 광고 도메인은 차단**됩니다.
